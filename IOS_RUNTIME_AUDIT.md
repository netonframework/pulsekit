...
XCArchive 中的 dSYM 现在会作为调试伴随文件解析，不会被误算为另一个随包 Framework。只有 dSYM
切片与发布镜像的 LC_UUID 和 architecture 同时一致时，内部函数地址表才会关联并入库；后台随后以
App ID、build number、LC_UUID 三重约束查询运行时事件，用镜像内偏移恢复被裁剪的调用点。最终页面按
Framework 汇总签名证书、`__TEXT` 指纹、运行时敏感 API、权限申请、Objective-C 方法清单、网络目的地
和调用路径。UUID 缺失、dSYM 缺失或 UUID 不匹配都会明确显示为未匹配，不使用文件名猜测。

静态符号是证据而不是源码还原：发布包裁剪符号后，纯 Swift/C/C++ 的本地定义可能不可见；匹配 dSYM
可恢复其保留的带地址函数符号，但当前不解析 DWARF 行表，因此不声称有源文件和行号。动态导入、load
command、UUID 和 `__TEXT` 指纹始终可用；Swift metadata 只能证明镜像含 Swift 类型信息，不能凭空
恢复未进入符号表的函数名。签名状态区分证书签名、ad-hoc、未签名和损坏签名，证书摘要来自最终 Code
Signature，不采信文件名或开发阶段工程配置。下一阶段才是更深的 Swift metadata 类型恢复、反汇编和
调用图；这些结果必须标注证据来源与置信度，不能把推断展示成真实执行记录。
...
全量拦截也会显著改变宿主性能。静态与运行时流水线现已通过 LC_UUID 精确关联；后续后台差异页需要把
相邻 build 的镜像路径、UUID、`__TEXT` 指纹、导入符号和证书集合做增删改对比，并将变化交给审计规则
生成告警。
